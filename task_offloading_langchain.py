...
def main():
    llm = ChatOpenAI(base_url="http://localhost:1234/v1", verbose=True, temperature=0.6, api_key='use-local-model')
    ...
    # TODO: Fix to run correct with `RunnableWithMessageHistory` instead of 
    conversation = RunnableWithMessageHistory(llm=llm, prompt=prompt, verbose=True)
