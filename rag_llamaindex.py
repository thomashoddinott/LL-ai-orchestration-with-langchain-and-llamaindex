...
os.environ["TOKENIZERS_PARALLELISM"] = (
    "false"  # workaround for HuggingFace FastTokenizers
)
...
def main():
    ...
    parser.add_argument(
        "--persist_dir",
        type=str,
        default="./handbook_index/",
        help="Path to store the serialized VectorStore",
    )
    ...
    if os.path.exists(args.persist_dir):
        ...
        vector_store = load_index_from_storage(storage_context=storage_context)
        print("done")
    else:
    ...
    chat_engine = ContextChatEngine.from_defaults(
        retriever=retriever, query_engine=query_engine, verbose=True
    )
